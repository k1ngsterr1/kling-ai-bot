  ...
  private async handleFetchImageCommand(chatId: number, taskId: string) {
    try {
      ...
      if (result && result.imageUrl) {
        ...
        // Enhanced image sending with multiple fallback strategies
        let imageSent = false;

        // First attempt: Send image directly by URL
        try {
          ...
          imageSent = true;
          this.logger.log('Image sent successfully via direct URL');
        } catch (directPhotoError) {
          this.logger.warn(
            'Direct photo send failed, trying buffer download:',
            directPhotoError.message,
          );

          // Second attempt: Download as buffer and send
          try {
            ...
            imageSent = true;
            this.logger.log('Image sent successfully via buffer download');
          } catch (bufferPhotoError) {
            this.logger.warn(
              'Buffer photo send also failed:',
              bufferPhotoError.message,
            );
          }
        }

        // Final fallback: Send as text message with download link
        if (!imageSent) {
          this.logger.warn(
            'All image sending methods failed, falling back to text message',
          );
          await this.bot.sendMessage(
            chatId,
    ...
    const checkImage = async () => {
      ...
      try {
        ...
        if (imageResult && imageResult.imageUrl) {
          ...
          try {
            this.logger.log(
              `📤 Attempting to send image: ${imageResult.imageUrl}`,
            );

            // Сначала проверим, доступен ли URL
            try {
              const axios = require('axios');
              const response = await axios.head(imageResult.imageUrl, {
                timeout: 10000,
                headers: {
              ...
              this.logger.log(
                `✅ Image URL is accessible (status: ${response.status})`,
              );
            } catch (urlError) {
              this.logger.warn(`⚠️ Image URL check failed:`, urlError.message);
            }
...
💡 Понравился результат? Создайте еще одно!`,
              reply_markup: {
                inline_keyboard: [
                  [
                    {
            ...
            this.logger.log(`✅ Successfully sent image as photo`);
            ...
            this.logger.error('❌ Failed to send image as photo:', imageError);
            this.logger.error(`   Error type: ${imageError.constructor.name}`);
            this.logger.error(`   Error message: ${imageError.message}`);

            // Попытка загрузить изображение и отправить как InputFile
            try {
              this.logger.log(`🔄 Trying to download and send as buffer...`);
              ...
              this.logger.log(
                `📥 Downloaded image buffer (${imageBuffer.length} bytes)`,
              );
...
💡 Понравился результат? Создайте еще одно!`,
              ...
              this.logger.log(`✅ Successfully sent image as buffer`);
              ...
              this.logger.error(
                '❌ Failed to send image as buffer:',
                bufferError,
              );

              // В крайнем случае отправляем ссылку
              this.logger.warn('📝 Sending as text message with download link');
              await this.bot.sendMessage(
                chatId,
...
⚠️ Возникла проблема с отображением изображения в чате. Вы можете скачать его по ссылке выше.`,
                {
                  reply_markup: {
                    inline_keyboard: [
                      [
            ...
            if (result && result.imageUrl) {
              ...
              try {
                ...
                try {
                  ...
                  imageSent = true;
                  this.logger.log(
                    'Image sent successfully via buffer download',
                  );
                  return;
                } catch (bufferPhotoError) {
                  this.logger.warn(
